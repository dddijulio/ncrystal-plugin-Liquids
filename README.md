...
`LIQUIDS`, but this should of course be updated to something more
appropriate. Once a suitable name has been chosen, be sure to update this in the
files `ncplugin_name.txt` and `pyproject.toml`, and replace `LIQUIDS` with
the name you have chosen. Additionally, the `@CUSTOM_LIQUIDS` section in any
...
@CUSTOM_LIQUIDS section with two numbers (the cross section below the
...
`NCRYSTAL_PLUGIN_DATADIRS` (after `LIQUIDS@`). However, a much more
