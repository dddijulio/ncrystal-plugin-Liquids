...
namespace NCPluginNamespace {
  ...
  //to be copied.
  struct LiquidInfo {
    std::string coherent_model = "";
    double c = 0;
    double wt = 0;
    double ws = 0;
    double cfrac = 0;  // coherent fraction: sigma_coh/(sigma_coh+sigma_inc)
    NC::VectD qval;
    NC::VectD Sval;
    std::string yk_model = "";  // "ORTHO" or "PARA" (empty = no YK rotational model)
  };
  ...
  public:
    ...
    //NC::SABData convoluteSABData(const NC::SABData& sabdata, const NC::DI_VDOS* di_vdos, double c_diff, double wt_diff);
  ...
  private:
    ...
    double m_wt;
    double m_wt_solid;
    std::unordered_map<std::string, LiquidInfo> m_liquid_data;
